use super::{PoseidonEngine, PoseidonHashParams};
use super::StatefulPoseidon;
...
#[derive(Clone)]
pub struct PoseidonTranscriptForRNS<'a, E: PoseidonEngine> {
    state: StatefulPoseidon<'a, E>,
    rns_parameters: &'a RnsParameters<E, <<E as Engine>::G1Affine as CurveAffine>::Base>
}


impl<'a, E: PoseidonEngine> Prng<E::Fr> for PoseidonTranscriptForRNS<'a, E> {
    type Input = E::Fr;
    ...
    fn new_from_params(params: Self::InitializationParameters) -> Self {
        let (poseidon_params, rns_params) = params;
        let stateful = StatefulPoseidon::new(poseidon_params);
...
impl<'a, E: PoseidonEngine> Transcript<E::Fr> for PoseidonTranscriptForRNS<'a, E> {
