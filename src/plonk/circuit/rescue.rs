...
use crate::bellman::plonk::better_better_cs::cs::{
    ArithmeticTerm, ConstraintSystem, MainGateTerm, PlonkConstraintSystemParams, Variable,
};
...
use crate::rescue::{
    PowerSBox, QuinticSBox, RescueEngine, RescueHashParams, RescueParamsInternal, SBox,
};
...
impl<E: Engine> PlonkCsSBox<E> for QuinticSBox<E> {
    ...
    fn apply_constraints<CS: ConstraintSystem<E>>(
        ...
        if force_no_custom_gates == false
...
impl<E: Engine> PlonkCsSBox<E> for PowerSBox<E> {
    ...
    fn apply_constraints_in_reverse<CS: ConstraintSystem<E>>(
        ...
        let ret = match el {
            ...
            Num::Variable(el) => {
                let out = AllocatedNum::<E>::alloc(cs, || {
                ...
                let fifth_term = ArithmeticTerm::from_variable(quad.get_variable())
                    .mul_by_variable(out.get_variable());
...
pub fn rescue_hash<E: RescueEngine, CS: ConstraintSystem<E>>(
    cs: &mut CS,
    params: &E::Params,
    input: &[Num<E>],
    ...
    <<E as RescueEngine>::Params as RescueHashParams<E>>::SBox1: PlonkCsSBox<E>,
{
    ...
    rescue_gadget.absorb_nums(cs, &input, &params)?;

    rescue_gadget.pad_if_necessary(&params)?;

    let mut result = vec![];
    for _ in 0..params.rate() {
        let res_lc = rescue_gadget.squeeze_out_single(cs, &params)?;
    ...
    <<E as RescueEngine>::Params as RescueHashParams<E>>::SBox1: PlonkCsSBox<E>,
{
    ...
    pub fn rescue_mimc_over_nums<CS: ConstraintSystem<E>>(
        ...
        let state: Vec<_> = state
            .iter()
            .map(|el| LinearCombination::from(*el))
            .collect();
        let out_lcs = Self::rescue_mimc_over_lcs(cs, &state, &params)?;
        let mut out = vec![];
        ...
        match self.mode {
            RescueOpMode::AccumulatingToAbsorb(ref mut into) => {
                ...
                if into.len() < rate {
                    ...
                    self.internal_state =
                        Self::rescue_mimc_over_lcs(cs, &self.internal_state, &params)?;
    ...
    pub fn absorb<CS: ConstraintSystem<E>>(
        ...
        for (_idx, val) in it.enumerate() {
            self.absorb_single_value(cs, val, &params)?;
        }
        ...
        match self.mode {
            RescueOpMode::AccumulatingToAbsorb(ref mut into) => {
                ...
                let output = sponge_output
                    .drain(0..1)
                    .next()
                    .expect("squeezed sponge must contain some data left");
    ...
    fn test_rescue_hash_plonk_gadget() {
...

        {
            ...
            let input_words: Vec<AllocatedNum<Bn256>> = input
                .iter()
                .enumerate()
                .map(|(_i, b)| AllocatedNum::alloc(&mut cs, || Ok(*b)).unwrap())
                .collect();
            ...
            rescue_gadget
                .absorb(&mut cs, &input_words, &params)
                .unwrap();

            let res_0 = rescue_gadget.squeeze_out_single(&mut cs, &params).unwrap();

            assert_eq!(res_0.get_value().unwrap(), expected[0]);
            println!(
                "Rescue stateful hash of {} elements taken {} constraints",
                input.len(),
                cs.n()
            );

            let res_1 = rescue_gadget.squeeze_out_single(&mut cs, &params).unwrap();
