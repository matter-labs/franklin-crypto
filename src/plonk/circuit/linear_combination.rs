...
impl<E: Engine> LinearCombination<E> {
    ...
    pub fn add_assign_scaled(&mut self, other: &Self, scale: E::Fr) {
    ...
    pub fn enforce_zero<CS: ConstraintSystem<E>>(self, cs: &mut CS) -> Result<(), SynthesisError> {
        if let Some(value) = self.get_value() {
            assert!(
                value.is_zero(),
                "LC is not actually zero with value {}",
                value
            );
        }
    ...
    pub fn into_allocated_num<CS: ConstraintSystem<E>>(
        ...
        let final_var = AllocatedNum::alloc(cs, || Ok(*self.value.get()?))?;
    ...
    fn deduplicate_stable(terms: Vec<(E::Fr, Variable)>) -> Vec<(E::Fr, Variable)> {
    ...
    fn enforce_zero_using_next_step<CS: ConstraintSystem<E>>(
        ...
        if num_terms <= CS::Params::STATE_WIDTH {
            ...
            let cycles = ((terms.len() - CS::Params::STATE_WIDTH) + (CS::Params::STATE_WIDTH - 2))
                / (CS::Params::STATE_WIDTH - 1); // ceil
            ...
            let (mut next_step_var_in_chain, mut next_step_var_in_chain_value) = {
                ...
                let may_be_new_intermediate_value =
                    Self::evaluate_term_value(&*cs, &chunk, constant_term);
            ...
            for _ in 0..(cycles - 1) {
                ...
                let may_be_new_intermediate_value =
                    Self::evaluate_term_value(&*cs, &chunk, E::Fr::zero()).map(|val| {
                        // and
                        let mut tmp = val;
                        tmp.add_assign(
                            next_step_var_in_chain_value
                                .as_ref()
                                .expect("value must be known"),
                        );
            ...
            // we also make sure that chained variable only goes into the last term
            {
                ...
                let idx_of_last_linear_term =
                    range_of_linear_terms.last().expect("must have an index");
    ...
    pub fn unwrap_as_allocated_num(&self) -> Result<AllocatedNum<E>, SynthesisError> {
...
mod test {
    ...
    fn test_inscribe_linear_combination() {
        ...
        let variables: Vec<_> = (0..9)
            .map(|_| AllocatedNum::alloc(&mut assembly, || Ok(Fr::one())).unwrap())
            .collect();
            ...
            fn synthesize<CS: ConstraintSystem<Bn256>>(
                ...
                for len in vec![3, 4, 5] {
                    ...
                    for i in 1..=len {
                        ...
                        let var = AllocatedNum::alloc(cs, || Ok(value))?;
