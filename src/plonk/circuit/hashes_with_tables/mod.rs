...
use crate::plonk::circuit::allocated_num::{AllocatedNum, Num, STATE_WIDTH};
...
fn get_or_create_table<
    ...
    FN: FnOnce() -> LookupTableApplication<E>,
>(
    cs: &mut CS,
    table_name: &str,
    init_fn: FN,
...
trait NumExtension<E: Engine>: Sized {
    ...
    fn lc<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        input_coeffs: &[E::Fr],
        inputs: &[Self],
    ) -> Result<Self, SynthesisError>;
    fn lc_with_d_next<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        input_coeffs: &[E::Fr],
        inputs: &[Self],
    ) -> Result<Self, SynthesisError>;
    fn sum<CS: ConstraintSystem<E>>(cs: &mut CS, nums: &[Self]) -> Result<Self, SynthesisError>;
    fn sum_with_d_next<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        nums: &[Self],
    ) -> Result<Self, SynthesisError>;
    fn long_weighted_sum<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        vars: &[Self],
        base: &E::Fr,
    ) -> Result<Self, SynthesisError>;
        ...
        CS: ConstraintSystem<E>,
    {
        ...
        if inputs.iter().all(|x| x.is_constant()) {
            let value =
                inputs
                    .iter()
                    .zip(input_coeffs.iter())
                    .fold(E::Fr::zero(), |acc, (x, coef)| {
            ...
            match x {
                ...
                Num::Variable(x) => {
                    if vars.len() < STATE_WIDTH {
                        ...
                        coeffs.reverse();
                        vars.reverse();
                        let tmp = AllocatedNum::quartic_lc_with_cnst(
                            cs,
                            &coeffs[..],
                            &vars[..],
                            &constant_term,
        ...
        if vars.len() == STATE_WIDTH {
            ...
            match use_d_next {
                true => {
                    AllocatedNum::quartic_lc_eq_with_cnst(
                        cs,
                        &coeffs[..],
                        &vars[..],
                        &res_var,
                ...
                false => {
                    let temp = AllocatedNum::quartic_lc_with_cnst(
                        cs,
                        &coeffs[..],
                        &vars[..],
                        &constant_term,
...
impl<E: Engine> AllocatedNumExtension<E> for AllocatedNum<E> {
    ...
    fn general_lc_gate<CS: ConstraintSystem<E>>(
        ...
        let next_row_term_idx = CS::MainGate::range_of_next_step_linear_terms()
            .last()
            .unwrap();
