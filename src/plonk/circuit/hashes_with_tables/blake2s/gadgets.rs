...
impl<E: Engine> Blake2sGadget<E> {
    ...
    fn alloc_num_from_u64<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        n: Option<u64>,
    ) -> Result<Num<E>> {
    ...
    fn alloc_reg_from_u64<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        n: Option<u64>,
    ) -> Result<Reg<E>> {
    ...
    fn allocate_gate<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        gate_alloc_helper: GateAllocHelper<E>,
    ) -> Result<()> {
    ...
    fn g_ternary_addition_process<CS: ConstraintSystem<E>>(
        ...
        first_row.set_var(
            2,
            u64_to_ff(1 << (2 * CHUNK_SIZE)),
            y.decomposed.r2.clone(),
            true,
        );
        first_row.set_var(
            3,
            u64_to_ff(1 << (3 * CHUNK_SIZE)),
            y.decomposed.r3.clone(),
            true,
        );
    ...
    fn g_binary_addition_process<CS>(
        &self,
        cs: &mut CS,
        a: &Reg<E>,
        b: &Reg<E>,
    ...
    fn g_xor_rot_setup_impl_rot_8_16<CS>(
        &self,
        cs: &mut CS,
        a: &Reg<E>,
        b: &Reg<E>,
        rot: usize,
    ) -> Result<XorRotOutput<E>>
    where
        CS: ConstraintSystem<E>,
    {
    ...
    // x[i2], y[i2], z[i2], z - q[i0] * 8^[i0] - q[i1] * 8^[i1] = w1
    // x[i3], y[i3], z[i3], z - q[i0] * 8^[i0] - q[i1] * 8^[i1] - q[i2] * 8^[i2] = w2
    fn g_xor_rot_setup_impl_rot_7_12<CS>(
        &self,
        cs: &mut CS,
        a: &Reg<E>,
        b: &Reg<E>,
        ...
        CS: ConstraintSystem<E>,
    {
        ...
        let (q_i0, q_ch_rots) = if self.use_additional_tables {
            let q_i0 = Num::Variable(self.xor_rot(
                cs,
                &a.decomposed[start_idx],
                &b.decomposed[start_idx],
                rot % CHUNK_SIZE,
            )?);
            (q_i0, None)
        } else {
            let q_i0 =
                Num::Variable(self.xor(cs, &a.decomposed[start_idx], &b.decomposed[start_idx])?);
            let q_ch_rots = (
    ...
    fn g_xor_rot_process<CS>(
        &self,
        cs: &mut CS,
        x: &Reg<E>,
        y: &Reg<E>,
        xor_rot_data: XorRotOutput<E>,
        rot: usize,
    ) -> Result<()>
    where
        CS: ConstraintSystem<E>,
    ...
    fn var_xor_const<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        input: &DecomposedNum<E>,
        cnst: u64,
    ) -> Result<Reg<E>> {
        assert_ne!(cnst, 0);
        let zero = E::Fr::zero();
    ...
    fn var_xor_var<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        x: &DecomposedNum<E>,
        y: &DecomposedNum<E>,
    ...
    fn var_xor_var_with_multiplexing<CS>(
        &self,
        cs: &mut CS,
        x: &DecomposedNum<E>,
        y: &DecomposedNum<E>,
    ...
    fn apply_ternary_xor<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        a: &Reg<E>,
        b: &Reg<E>,
        ...
        let res = match ((a.is_const(), a), (b.is_const(), b), (c.is_const(), c)) {
            ...
            ((false, var_reg), (true, cnst_reg0), (true, cnst_reg1))
            | ((true, cnst_reg0), (false, var_reg), (true, cnst_reg1))
            ...
            ((false, var_reg0), (true, cnst_reg), (false, var_reg1))
            | ((true, cnst_reg), (false, var_reg0), (false, var_reg1))
            | ((false, var_reg0), (false, var_reg1), (true, cnst_reg)) => {
                let tmp = self.var_xor_var_with_multiplexing(
                    cs,
                    &var_reg0.decomposed,
                    &var_reg1.decomposed,
                )?;
    ...
    fn apply_xor_with_const<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        reg: &Reg<E>,
        cnst: u64,
        ...
        CS: ConstraintSystem<E>,
    {
        ...
        v.0[12] =
            self.apply_xor_with_const(cs, &mut v.0[12], total_len & ((1 << REG_WIDTH) - 1))?; // Low word of the offset.
        ...
        for i in 0..(BLAKE2S_STATE_WIDTH / 2) {
            // h[i] := h[i] ^ v[i] ^ v[i + 8]
            let t = self.apply_ternary_xor(
                cs,
                &hash_state.0[i],
                &v.0[i],
                &v.0[i + (BLAKE2S_STATE_WIDTH / 2)],
            )?;
            res.0.push(t);
        }
        Ok(res)
    ...
    pub fn digest_bytes<CS: ConstraintSystem<E>>(
        &self,
        cs: &mut CS,
        bytes: &[Byte<E>],
    ) -> Result<Vec<Num<E>>> {
        ...
        let num_of_zero_bytes = if last_block_size > 0 {
            64 - last_block_size
        } else {
            0
        };
