...
pub trait ChannelGadget<E: Engine> {
    ...
    fn consume<CS: ConstraintSystem<E>>(
        &mut self,
        data: AllocatedNum<E>,
        cs: &mut CS,
    ...
    fn produce_challenge<CS: ConstraintSystem<E>>(
        &mut self,
        cs: &mut CS,
    ...
    <<E as RescueEngine>::Params as RescueHashParams<E>>::SBox1: PlonkCsSBox<E>,
{
    ...
    fn consume<CS: ConstraintSystem<E>>(
        ...
        self.state
            .absorb_single_value(cs, Num::Variable(data), &self.params)?;
        ...
        if params.can_allocate_from_double_limb_witness() {
            ...
            for coord in vec![x, y] {
                ...
                for w in witnesses.into_iter() {
                    let selected =
                        AllocatedNum::conditionally_select(cs, &zero, &w, &data.get_zero_flag())?;
            ...
            for coord in vec![x, y] {
                for limb in coord.binary_limbs[..num_witness].iter() {
                    ...
                    let selected =
                        AllocatedNum::conditionally_select(cs, &zero, &l, &data.get_zero_flag())?;
