...
impl<E: Engine, T> CircuitSelectable<E> for std::marker::PhantomData<T> {
    fn conditionally_select<CS: ConstraintSystem<E>>(
        _cs: &mut CS, _flag: &Boolean, _a: &Self, _b: &Self
...
//     fn alloc_from_witness<CS: ConstraintSystem<E>>(_cs: &mut CS, _witness: Option<Self::Witness>) -> Result<Self, SynthesisError> {
//         Ok(std::marker::PhantomData)
//     }
// }
...
impl<E: Engine, T: CircuitSelectable<E>> CircuitSelectable<E> for Option<T> {
    fn conditionally_select<CS: ConstraintSystem<E>>(_cs: &mut CS, _flag: &Boolean, _a: &Self, _b: &Self) -> Result<Self, SynthesisError> {
        Ok(None)
    }
}
...
impl<E: Engine, T, const N: usize> CircuitEq<E> for [T; N] where T: CircuitEq<E> {
    fn eq(&self, other: &Self) -> bool {
        self.iter().zip(other.iter()).all(|(a, b)| a.eq(b))
    }
}

impl<E: Engine, T, const N: usize> CircuitOrd<E> for [T; N] where T: CircuitOrd<E> {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        for (a, b) in self.iter().zip(other.iter()) {
            match a.cmp(&b) {
                std::cmp::Ordering::Less => return std::cmp::Ordering::Less,
...
impl<E: Engine, T: CircuitSelectable<E>, const N: usize> CircuitSelectable<E> for [T; N] {
    fn conditionally_select<CS: ConstraintSystem<E>>(cs: &mut CS, flag: &Boolean, a: &Self, b: &Self) -> Result<Self, SynthesisError> {
        ...
        let result: [T; N] = result.try_into().unwrap_or_else(|_| panic!("invalid length"));
    ...
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (Num::Variable(a), Num::Variable(b)) => {
                <AllocatedNum<E> as CircuitEq<E>>::eq(&a, &b)
            },
            _ => {
                false
            }
...
impl<E: Engine> CircuitOrd<E> for Boolean{
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        match (self, other) {
            ...
            (Boolean::Constant(_a), _) => {
                std::cmp::Ordering::Less
            },
            (Boolean::Is(..), Boolean::Constant(..)) => {
                std::cmp::Ordering::Greater
            },
            (Boolean::Not(..), Boolean::Constant(..)) => {
                std::cmp::Ordering::Greater
            },
            (Boolean::Is(..), Boolean::Not(..)) => {
                std::cmp::Ordering::Less
            },
            (Boolean::Is(a), Boolean::Is(b)) => {
...
//             (Some(a), Some(b)) => {
//                 a.iter().zip(b.iter()).all(|(a, b)| <Boolean as CircuitEq<E>>::eq(a, b))
//             },
//             (None, None) => {
//                 true
//             },
...
pub trait CircuitOrthogonalSelectable<E: Engine>: CircuitEq<E> + CircuitOrd<E> + CircuitSelectable<E> + Clone {
    fn select_update_assuming_orthogonality<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        reference: Self,
...
//         candidates: &[(Boolean, Self)]
//     ) -> Result<Self, SynthesisError> {
//         let (_, partitions) = partition::<E, _, _>(
//             &reference, 
//             &candidates
//         );
...
impl<E: Engine, T: CircuitOrthogonalSelectable<E>, const N: usize> CircuitOrthogonalSelectable<E> for [T; N] {
    fn select_update_assuming_orthogonality<CS: ConstraintSystem<E>>(
        cs: &mut CS,
        reference: Self,
        candidates: &[(Boolean, Self)]
    ) -> Result<Self, SynthesisError> {
        array_select_update_assuming_orthogonality_per_element_strategy(
            cs,
            reference,
            candidates
        )
...
pub fn array_select_update_assuming_orthogonality_per_element_strategy<E: Engine, CS: ConstraintSystem<E>, T: CircuitOrthogonalSelectable<E>, const N: usize>(
    cs: &mut CS,
    reference: [T; N],
    ...
    let mut result = vec![];
    for i in 0..N {
        let mut subset: Vec<_> = candidates.iter().map(|el| (el.0, el.1[i].clone())).collect();
...
// pub fn array_select_update_assuming_orthogonality_per_full_array_strategy<E: Engine, CS: ConstraintSystem<E>, T: CircuitEq<E> + CircuitOrd<E> + CircuitSelectable<E> + Clone, const N: usize>(
//     cs: &mut CS,
//     reference: [T; N],
//     candidates: &[(Boolean, [T; N])]
// ) -> Result<[T; N], SynthesisError> {
