...
impl IntegerPermutation {
    ...
    pub fn new_from_permutation(permutation: Vec<usize>) -> Self {
        assert!(permutation.len() > 0);
        let min = *permutation.iter().min().expect("must have a min element");
        let max = *permutation.iter().max().expect("must have a max element");

        let mut sorted_copy = permutation.clone();
        sorted_copy.sort();
        let mut prev = sorted_copy.drain(0..1).next().expect("must contain an element");
