...
impl UInt32 {
    ...
    pub fn alloc<E: Engine, CS: ConstraintSystem<E>>(
        ...
        let bits = values
            .into_iter()
            .enumerate()
            .map(|(_i, v)| Ok(Boolean::from(AllocatedBit::alloc(cs, v)?)))
        ...
        CS: ConstraintSystem<E>,
    {
        Self::triop(
            ...
            c,
            |a, b, c| (a & b) ^ (a & c) ^ (b & c),
            |cs, _i, a, b, c| Boolean::sha256_maj(cs, a, b, c),
        )
    }
        ...
        CS: ConstraintSystem<E>,
    {
        Self::triop(
            ...
            c,
            |a, b, c| (a & b) ^ ((!a) & c),
            |cs, _i, a, b, c| Boolean::sha256_ch(cs, a, b, c),
        )
    }
    ...
    pub fn addmany<E, CS>(cs: &mut CS, operands: &[Self]) -> Result<Self, SynthesisError>
    where
        E: Engine,
        CS: ConstraintSystem<E>,
    {
        ...
        let mut multieq_gadget = MultiEq::new(&mut *cs);
        ...
        multieq_gadget.enforce_equal(i, &lc, &result_lc);

        drop(multieq_gadget); // this will synthesize constraints
...
mod test {
    ...
    fn test_uint32_xor() {
        ...
        for _ in 0..1000 {
            ...
            let a_bit = UInt32::alloc(&mut cs, Some(a)).unwrap();
            let b_bit = UInt32::constant(b);
            let c_bit = UInt32::alloc(&mut cs, Some(c)).unwrap();

            let r = a_bit.xor(&mut cs, &b_bit).unwrap();
            let r = r.xor(&mut cs, &c_bit).unwrap();
    ...
    fn test_uint32_addmany_constants() {
        ...
        for _ in 0..1000 {
            ...
            let r = {
                ...
                let r = UInt32::addmany(cs.as_cs(), &[a_bit, b_bit, c_bit]).unwrap();
                r
            };
    ...
    fn test_uint32_addmany() {
        ...
        for _ in 0..1000 {
            ...
            let b_bit = UInt32::constant(b);
            let c_bit = UInt32::constant(c);
            let d_bit = UInt32::alloc(&mut cs, Some(d)).unwrap();

            let r = a_bit.xor(&mut cs, &b_bit).unwrap();
            let r = {
                let mut cs = MultiEq::new(&mut cs);
                let r = UInt32::addmany(cs.as_cs(), &[r, c_bit, d_bit]).unwrap();
                r
            };
